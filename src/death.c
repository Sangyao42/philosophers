...
/*   Updated: 2023/05/09 13:44:03 by sawang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_kill(struct s_table *table)
{
	...
	while (1)
	{
		...
		while (i < table->input.num_of_philos)
		{
			if (check_dead(&table->philo_holding.philos[i]) == true)
				return (set_kill(table), print_status(&table->philo_holding.\
				philos[i], "died"), \
		...
		if (check_eaten(table) == true)
			return (set_kill(table), print_status(&table->philo_holding.\
			philos[i - 1], "all philos ate enough"), \
			philo_threads_join(table, table->input.num_of_philos), NULL);
	}
}
