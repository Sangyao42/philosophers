...
/*   Updated: 2023/04/27 16:28:52 by sawang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	set_philo_holding(struct s_table *table)
{
	...
	// table->philo_holding.philo_thrs = (pthread_t *)(\
	// 	table->philo_holding.philos + table->input.num_of_philos);
	// table->philo_holding.mutex_forks = (pthread_mutex_t *)(\
	// 	table->philo_holding.philo_thrs + table->input.num_of_philos);
	table->philo_holding.philos = ft_calloc(table->input.num_of_philos,  (sizeof(struct s_philo)));
	table->philo_holding.philo_thrs = ft_calloc(table->input.num_of_philos, (sizeof(pthread_t)));
	table->philo_holding.mutex_forks = ft_calloc(table->input.num_of_philos, (sizeof(pthread_mutex_t)));
	if (!table->philo_holding.philos || !table->philo_holding.philo_thrs || !table->philo_holding.mutex_forks)
		return (printf("Error: Malloc failed"), i);
	while (i < table->input.num_of_philos)
	{
		if (pthread_mutex_init(&table->philo_holding.mutex_forks[i], NULL) \
			!= 0)
			return (printf("Error: %dth Mutex init failed", i), i);
		i++;
	}
	return (i);
}
...
bool	set_table(struct s_table *table, int argc, char *argv[])
{
	...
	if (set_traffic_light(table) == EXIT_FAILURE)
		return (mutex_destroy_and_free_when_init(table, 0), EXIT_FAILURE);
	philo_cnt = set_philo_holding(table);
	if (philo_cnt != table->input.num_of_philos)
	{
		mutex_destroy_and_free_when_init(table, philo_cnt);
		return (EXIT_FAILURE);
	}
	sit_philos(table);
	return (EXIT_SUCCESS);
